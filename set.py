    ...
    try:
        if Flag==False:
            for box in main_box:
                ...
                if product.lower() in val.lower():
                    price=box.find("span",{"class":"a-price-whole"}).text.strip()
                    temp.append([l,val,price])
        else:
            for box in main_box:
                s="https://www.amazon.in"
...
def flip_app_price(product):
    ...
    response = requests.get(url, headers=headers_flip)
    soup= BeautifulSoup(response.text,'html.parser')
    main_box= soup.find_all('div',{"class":"IIdQZO _1SSAGr"})
    box=main_box[0]
    temp=[]
    try:
        for box in main_box:
...
def getValue():
    ...
    if choice=="tech":
        ...
        #if words[:] in list(product_name):
         #   return render_template('show_product.html', p=product_name, li_amaz=amaz_list)
        #else:
